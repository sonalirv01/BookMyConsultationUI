...
const Register = (props) => {
  ...
  const formInputFields = [
    ...
    { name: "emailId", label: "Email Id", required: true, type: "email", autoFocus: false },
    { name: "password", label: "Password", required: true, type: "password", autoFocus: false },
    { name: "mobile", label: "Mobile No", required: true, type: "text", autoFocus: false },
  ];
  ...
  const validate = (values) => {
    ...
    if (errors.emailId.isValid && !regexEmail.test(values.emailId)) {
      errors.emailId = { isValid: false, errMsg: "Enter valid email", isEmpty: false };
    }
    if (errors.password.isValid && values.password.length < 4) {
      errors.password = { isValid: false, errMsg: "Password length less than 4", isEmpty: false };
    }
    if (errors.mobile.isValid && !regexPhone.test(values.mobile)) {
      errors.mobile = { isValid: false, errMsg: "Enter valid mobile number", isEmpty: false };
    }
    return errors;
  };
  ...
  return (
    <div>
      <form onSubmit={handleSubmit} noValidate autoComplete="off" className={classes.centerElement}>
        ...
        <br />
        {registerFailed && <Typography color="error">Registration Failed!</Typography>}
        {registerSuccess && (
